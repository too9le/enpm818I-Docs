%%% SVN stuff
\svnidlong
{$HeadURL: https://192.168.1.5/svn/NASC/_Projects/High%20Gain%20Array%20-%20HGA/SystemDocuments/3%20-%20SSS/SSS_doc_history.tex $}
{$LastChangedDate: 2017-01-22 21:21:30 -0500 (Sun, 22 Jan 2017) $}
{$LastChangedRevision: 468 $}
{$LastChangedBy: LCollier $}
\svnid{$Id: SSS_doc_history.tex 468 2017-01-23 02:21:30Z LCollier $}

The following table is a simple list of released revisions sent for review.
Records of reviews and the review artifacts are saved with reviewer information in the \KNEADagencyName artifact repository.

\begin{table}[h]
	\centering 
		\begin{tabular}{|p{1.0in}|p{0.8in}|p{1.4in}|p{2.1in}|}
    \multicolumn{4}{l}{\bfseries Change Record} \\
		\hline
			{\bfseries Date}  &  {\bfseries Version} & {\bfseries Author(s)} & {\bfseries Change Reference} \\
		\hline
		\hline
			 01 Feb 2021	&	\centering	P1	&	{\raggedright Lewis Collier and Brady Spatola}	&	Preliminary DRAFT version  \\ \hline
			 25 Mar	2022	&	\centering	P2	&	{\raggedright Lewis Collier, Chris Doerge, and Brady Spatola}	&	Numerous additions made throughout the document \\ \hline
			 14 Apr	2022	&	\centering	P3	&	{\raggedright Lewis Collier, Chris Doerge, and Brady Spatola}	&	Changes made throughout the document \\
		\hline		\hline				
  	\end{tabular}
	%%%%%\caption{Change Record}
	%%%%%\label{tab:ChangeRecord}
\end{table}

Each subsequent ``section'' outlines changes in each release.

Items in this version that are marked with change bars have been modified from the most recent previous version (e.g. P3 changes from P2) or are new as of the current revision. A list of all changed items may be found in the Index section under the heading ``All Changes This Version''.

{\bf Draft P1}
Preliminary version of this document.

{\bf Draft P2}
Second draft version of this document with a number additions from P1.

{\bf Draft P3}
Third draft version of this document with a number changes and additions from P2.

